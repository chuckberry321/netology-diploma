...
resource "null_resource" "copy_kube_config" {
  provisioner "local-exec" {
    command = "scp -i /tmp/id_rsa_tf -o 'StrictHostKeyChecking no' ubuntu@${yandex_compute_instance.master[0].network_interface.0.nat_ip_address}:/home/ubuntu/.kube/config $HOME/.kube/config"
  }
