...
resource "null_resource" "copy_cluster_config" {
  provisioner "local-exec" {
    command = "cp -r ../ansible/netology-k8s-cluster/ /tmp/kubespray/inventory/ && ls -la /tmp/kubespray/inventory/netology-k8s-cluster/local/ && ls -la /tmp/kubespray/" 
  }
...
resource "null_resource" "config_netology_k8s_cluster" {
  provisioner "local-exec" {
    command = "ANSIBLE_FORCE_COLOR=1 ansible-playbook -i /tmp/kubespray/inventory/netology-k8s-cluster/local/ /tmp/kubespray/cluster.yml -b -vvvvv --flush-cache"
  }
